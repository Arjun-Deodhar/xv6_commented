 ...
 * frmo the global ptable data structure
 */
static struct proc*
allocproc(void)
{
...
80103816:	8d 90 b4 0f 00 00    	lea    0xfb4(%eax),%edx
  ...
  /* inserts some assembly language instruction to copy a sequence of
   * bytes from one lcoation to another
   */
...
801043d0 <initlock>:
 * initialises a lock lk, with name "name"
 * locked, cpu pointers set to NULL
 */
void
...
80105860:	55                   	push   %ebp
   ...
   * 	the gate_desc is set to a syscall by passing istrap = 1
   * 	which defines privilege = 3 which is user
   */
  for(i = 0; i < 256; i++)
